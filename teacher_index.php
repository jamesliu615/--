...
function randomFillTable() {
  ...
  for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < columns; j++) {
          ...
          rowData.push({ 學號: selectedStudent.學號, 姓名: selectedStudent.姓名, 第幾排: rowIndex, 第幾個: columnIndex });
      }
  ...
  document.getElementById("cancelExchangeButton").addEventListener("click", cancelExchange);
    ...
    cell.student = { 學號: selectedStudent.學號, 姓名: selectedStudent.姓名 };
...
function saveTableData() {
  ...
  // 发送请求保存数据
  axios.post('座位資料表.php', { tableData })
    .then(response => {
      Swal.fire({
        title: '成功!',
        text: '表格數據已成功保存到資料庫。',
