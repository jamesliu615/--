                ...
                <div class="col-6">
                  <div class="table-generator">
                    ...
                    <div>
                      <label for="tableSizeSelect">選擇座位表大小：</label>
                      <select id="tableSizeSelect" onchange="updateTableSize()">
                        <option value="3x3">3行 x 3列</option>
                        <option value="4x4">4行 x 4列</option>
                        <option value="5x5">5行 x 5列</option>
                        <option value="6x6">6行 x 6列</option>
                        <!-- 更多行列組合選項 -->
                      </select>
                    </div>
                    <div class="buttons-container">
                      ...
                      <button id="exchangeButton" class="btn btn-warning">座位交換</button>
        ...
        `;
                          ...
                          checkbox.addEventListener('click', (event) => {
                            if (event.target.checked) {
                              ...
                              const indexToRemove = selectedStudentData.findIndex(selected => selected.學號 === student.學號);
                              if (indexToRemove !== -1) {
                      ...
                      function randomFillTable() {
                        ...
                        let studentsForFilling = [...selectedStudentData];
                        ...
                        for (let i = 0; i < rows; i++) {
                          ...
                          for (let j = 0; j < columns; j++) {
                            ...
                            const selectedStudent = studentsForFilling.splice(randomIndex, 1)[0];
                            const rowIndex = i + 1;
                            const columnIndex = j + 1;

                            cell.innerHTML = `學號: ${selectedStudent.學號}<br>姓名: ${selectedStudent.姓名}<br>（第${rowIndex}排第${columnIndex}個）`;
                            ...
                            rowData.push({
                              ...
                              第幾個: columnIndex,
                              student: {
                                學號: selectedStudent.學號,
                                姓名: selectedStudent.姓名
                              }
                            });
                          }
                      ...
                      function updateTableSize() {
                        ...
                        for (let i = 0; i < rows; i++) {
                          ...
                          for (let j = 0; j < columns; j++) {
                            ...
                            toggleButton.textContent = '切換狀態';
                            toggleButton.addEventListener('click', toggleCellState);
                      ...
                      function toggleCellState(event) {
                        ...
                        const addButton = cell.querySelector('.add-student-button');

                        if (addButton.disabled) {
                          addButton.disabled = false;
                          addButton.textContent = '加入學生';
                          cell.style.backgroundColor = ''; // 可選：改變背景顏色來反映狀態
                        } else {
                          addButton.disabled = true;
                          addButton.textContent = '無法加入';
                          cell.style.backgroundColor = 'lightgray'; // 可選：改變背景顏色來反映狀態
                        }
                      }

                      function addDeleteButtonToCell(cell) {
                        const deleteButton = document.createElement('button');
                        deleteButton.textContent = '刪除';
                        deleteButton.className = 'btn btn-danger btn-sm'; // Bootstrap樣式
                      ...
                      function addDeleteButton(cell) {
                        const deleteButton = document.createElement('button');
                        deleteButton.textContent = '刪除';
                        deleteButton.className = 'btn btn-danger btn-sm'; // Bootstrap样式
                        deleteButton.onclick = function() {
                          cell.innerHTML = ''; // 清空單元格内容
                        };
                        cell.appendChild(deleteButton);
                      }
                      ...
                      function addStudentToCell(event) {
                        ...
                        if (!validateSelectedStudents()) {
                          return; // validateSelectedStudents 函数会显示错误信息
                        }

                        // 防呆：检查是否已生成表格
                        if (!validateTableExists()) {
                          return; // validateTableExists 函数会显示错误信息
                        }

                        if (rowIndex <= 0 || colIndex <= 0 || rowIndex > tableData.length || colIndex > tableData[0].length) {
                          Swal.fire({
                            title: '錯誤!',
                            text: '無效的行或列索引。',
                            icon: 'error',
                        ...
                        const selectedStudent = selectedStudentData.splice(randomIndex, 1)[0]; // 從已選擇的學生列表中移除學生

                        const cell = tableData[rowIndex - 1][colIndex - 1];
                        const cellElement = cell.button.parentElement;

                        // 將學生資訊添加到單元格
                        cellElement.innerHTML = `學號: ${selectedStudent.學號}<br>姓名: ${selectedStudent.姓名}<br>（第${rowIndex}排第${colIndex}個）`;
                        cell.student = {
                          學號: selectedStudent.學號,
                          姓名: selectedStudent.姓名
                        };
